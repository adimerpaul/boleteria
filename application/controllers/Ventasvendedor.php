...
class Ventasvendedor extends CI_Controller
{
    ...
    public function totallistaperiodo(){
        ...
        $query=$this->db->query("SELECT 
         ...
         (select sum(costo) from boleto b1 inner join funcion f on b1.idFuncion= f.idFuncion where date(b1.fecha)>= '$fecini' and date(b1.fecha)<='$fecfin' and idPelicula in ".$peliculas."  and devuelto='NO') as totalventa,
         ...
         (select sum(costo) from boleto b1 inner join funcion f on b1.idFuncion= f.idFuncion where date(b1.fecha)>= '$fecini' and date(b1.fecha)<='$fecfin' and idPelicula in ".$peliculas."  and devuelto='SI') as totaldev 
         from dual");
                 $row=$query->row();
                 $myObj=($query->result_array());
                 echo json_encode($myObj);  
    ...
    public function totallistaperiodo2(){
        ...
        $query=$this->db->query("SELECT 
         ...
         (select sum(costo) from boleto b1 inner join funcion f on b1.idFuncion= f.idFuncion where idPelicula in ".$peliculas."  and devuelto='NO') as totalventa,
         ...
         (select sum(costo) from boleto b1 inner join funcion f on b1.idFuncion= f.idFuncion where  idPelicula in ".$peliculas."  and devuelto='SI') as totaldev 
         from dual");
                 $row=$query->row();
                 $myObj=($query->result_array());
                 echo json_encode($myObj);  
    ...
    public function porsemana2(){
        ...
        $consulta="SELECT p.idPelicula,concat(p.nombre,' ',if(p.formato=1,'3D','2D')) as titulo,(
            ...
            INNER JOIN funcion f1 ON b1.idFuncion=f1.idFuncion
            INNER JOIN pelicula p1 ON p1.idPelicula=f1.idPelicula
            WHERE WEEKDAY(date(b1.fecha))+1=0
            AND p1.idPelicula=p.idPelicula
            ) as lunes,
            (
            SELECT count(*)
            ...
            (SELECT sum(precio)
             ...
             INNER JOIN pelicula p1 ON p1.idPelicula=f1.idPelicula
             inner join tarifa t on t.idTarifa = f1.idTarifa
             WHERE p1.idPelicula=p.idPelicula
             ) as ingreso,
            count(*) as total            
            FROM boleto b  
            INNER JOIN funcion f ON b.idFuncion=f.idFuncion
            INNER JOIN pelicula p ON p.idPelicula=f.idPelicula
            WHERE date(f.fecha)>=date('$fecini') AND date(f.fecha)<=date('$fecfin')
            and p.idPelicula in ".$peliculas." group by p.idPelicula ";
