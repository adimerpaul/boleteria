...
<script>
    ...
    window.onload=async function (e) {
        await verificardosificacion();
        await mostrarerrores();
        if(errores.length==0){
            await mostrarpeliculas();
        }
        async function verificardosificacion(){
            await $.ajax({
                url:'Ventarapida/verificardosificacion',
                success:function (e) {
                    let dato=JSON.parse(e);
                    if (dato.length==0){
                        errores.push('No exite  una dosificacion');
                    }else if(dato.length>1){
                        errores.push('Existen muchas dosificaciones');
                    }else{
                        var fechainicial = moment().format("YYYY-MM-DD") ;

                        var fechafinal = dato[0].fechaHasta;
                        // console.log(dato[0].fechaHasta);
                        if(Date.parse(fechafinal) < Date.parse(fechainicial)){
                            //La fecha final es menor que la inicial
                            errores.push('La fecha de la doficicacion no es valida');
                        }
                    }
                }
            })
        ...
        function mostrarerrores(){
            $('#contenedorerrores').html('');
            errores.forEach(e=>{
                // console.log(e);
                $('#contenedorerrores').append("" +
        ...
        async function mostrarpeliculas(){
                ...
                $("#selecfun").html("");
                await $.ajax({
                    data:  'fecha1='+$('#fecfuncion').prop('value'),
                    ...
                    success:  function (response) {
                        ...
                        datos.forEach(row => {
                            if(row.formato == 1) dd="3D";
                                ...
                                cadenapelicula=cadenapelicula+'<li class="ui-widget-content" style="background-image:  url('+'img/sonic.jpg'+');opacity: 0.5" value="'+row.idPelicula+'" > ';
        ...
        $("#selectable").selectable({
                stop: function(){
                    var id=0;
                    $(".ui-widget-content").css('opacity', '0.5');
                    $( ".ui-selected", this ).each(async function() {
                        $(this).css('opacity', '1');
                        ...
                        await $.ajax({
                            ...
                            success:  function (response) {
                                ...
                                datos.forEach(row => {
                                    ...
                                    cadenahorario=cadenahorario+'<h6 >'+ row.porcentaje+'</h6>';
